...
export module Spark.Editor.UI.Properties.PropertyEditor;
import Spark.UI.Anchors;
...
{
    SPARK_ENUM()
    export enum class PropertyTitleMode
    {
        None,
        Inline,
        NextLine
    ...
    export std::unordered_map<reflect::Type*, reflect::Type*>& GetPropertyEditorTypes();
    export std::unordered_map<std::string, reflect::Type*>& GetContainerPropertyEditorTypes();

    export struct PropertyEditorRegister
    {
...
#define SSCANF(buffer, str, var, ...) sscanf_s(buffer, str, var, __VA_ARGS__)
    ...
    export struct PropertyEditorParams
    {
        std::string name = {};
    ...
    export class PropertyEditor : public reflect::ObjectBase, public std::enable_shared_from_this<PropertyEditor>
    {
    public:
