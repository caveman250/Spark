    ...
    void AABBColliderSystem::OnUpdate(const ecs::QueryResults& results)
    {
        ecs::ForEachArcheType(results, ecs::UpdateMode::MultiThreaded, false, [](const ecs::SystemUpdateData& updateData)
        {
            ...
            for (size_t i = 0; i < entities.size(); ++i)
            {
                ...
                collisionComp->colliders.insert(std::make_pair(entity, singleton_components::ColliderRecord{ collider.dynamic, collider.aabb, transform.pos, collider.collisionChannel.channel }));
