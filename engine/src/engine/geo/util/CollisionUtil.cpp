...
namespace se::geo::util
{
    std::optional<RayCastHit> RayCastAABB(const Ray& ray, const ecs::components::TransformComponent& transform)
    {
        ...
        const math::Vec3 min = (transform.aabb.center - transform.aabb.size / 2 - localSpaceRay.origin) / localSpaceRay.direction;
        const math::Vec3 max = (transform.aabb.center + transform.aabb.size / 2 - localSpaceRay.origin) / localSpaceRay.direction;
