...
namespace se::render
{
    void Material::Bind(const VertexBuffer& vb)
    {
        ...
        if (m_RenderState.lit)
        {
            ...
            {
                m_ShaderSettings.SetSetting("numDirLights", static_cast<int>(lightSetup.dirLights.size()), true);
                m_ShaderSettings.SetSetting("numPointLights", static_cast<int>(lightSetup.pointLights.size()), true);
